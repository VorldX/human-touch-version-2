...
export function SettingsConsole({ orgId, themeStyle, initialLane }: SettingsConsoleProps) {
  ...
  const loadSettings = useCallback(
    async (silent?: boolean) => {
      ...
      try {
        const [webhooksRes, railsRes, identityRes, llmRes] = await Promise.all([
  ...
  return (
    <div className="mx-auto max-w-[1280px] space-y-6">
      ...
      {loading ? (
        ...
        <form onSubmit={saveOrchestration} className={`vx-panel space-y-3 rounded-3xl p-4 ${themeStyle.border}`}>
          <div className="grid gap-2 md:grid-cols-2">
            <label className="text-xs text-slate-300">Mode
              <select value={llmSettings.mode} onChange={(event) => setLlmSettings((prev) => ({ ...prev, mode: event.target.value as RuntimeMode }))} className="mt-1 w-full rounded-xl border border-white/10 bg-black/40 px-3 py-2 text-sm text-slate-100 outline-none"><option value="BYOK">BYOK</option><option value="PLATFORM_MANAGED">PLATFORM_MANAGED</option></select>
            </label>
            <label className="text-xs text-slate-300">Plan
              <select value={llmSettings.servicePlan} onChange={(event) => setLlmSettings((prev) => ({ ...prev, servicePlan: event.target.value as ServicePlan, serviceMarkupPct: defaultMarkup(event.target.value as ServicePlan) }))} className="mt-1 w-full rounded-xl border border-white/10 bg-black/40 px-3 py-2 text-sm text-slate-100 outline-none"><option value="STARTER">STARTER</option><option value="GROWTH">GROWTH</option><option value="ENTERPRISE">ENTERPRISE</option></select>
            </label>
          </div>
          <div className="grid gap-2 md:grid-cols-2">
            <input value={llmSettings.provider} onChange={(event) => setLlmSettings((prev) => ({ ...prev, provider: event.target.value }))} placeholder="Primary provider" className="rounded-xl border border-white/10 bg-black/40 px-3 py-2 text-sm text-slate-100 outline-none" />
            <input value={llmSettings.model} onChange={(event) => setLlmSettings((prev) => ({ ...prev, model: event.target.value }))} placeholder="Primary model" className="rounded-xl border border-white/10 bg-black/40 px-3 py-2 text-sm text-slate-100 outline-none" />
            <input value={llmSettings.fallbackProvider} onChange={(event) => setLlmSettings((prev) => ({ ...prev, fallbackProvider: event.target.value }))} placeholder="Fallback provider" className="rounded-xl border border-white/10 bg-black/40 px-3 py-2 text-sm text-slate-100 outline-none" />
            <input value={llmSettings.fallbackModel} onChange={(event) => setLlmSettings((prev) => ({ ...prev, fallbackModel: event.target.value }))} placeholder="Fallback model" className="rounded-xl border border-white/10 bg-black/40 px-3 py-2 text-sm text-slate-100 outline-none" />
          </div>
          <input type="number" min={0} max={200} step={0.1} value={llmSettings.serviceMarkupPct} onChange={(event) => setLlmSettings((prev) => ({ ...prev, serviceMarkupPct: Number(event.target.value) || 0 }))} className="w-full rounded-xl border border-white/10 bg-black/40 px-3 py-2 text-sm text-slate-100 outline-none" />
          <div className="grid gap-2 md:grid-cols-2">
          ...
          <button type="submit" disabled={savingLlm} className="inline-flex items-center gap-2 rounded-full bg-white px-5 py-2 text-[10px] font-bold uppercase tracking-[0.2em] text-black transition hover:bg-emerald-500 hover:text-white disabled:opacity-60">{savingLlm ? <Loader2 size={14} className="animate-spin" /> : null}Save Orchestration Settings</button>
        </form>
      )}
    </div>
  );
